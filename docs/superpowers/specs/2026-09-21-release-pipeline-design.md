...
3. Merge entries from the old hand-written file, rewritten in the generated
   ...
   everything from the old file; for 2.0.0 and 3.0.0, the breaking-change
   items; for other versions, nothing (generated content wins).
4. Header: a short line stating the file is generated by
   `conventional-changelog` (angular preset); the Keep a Changelog / SemVer
   preamble, the `[Unreleased]` section and the trailing `[x.y.z]: compare`
   link list are removed (headings carry the compare links).
